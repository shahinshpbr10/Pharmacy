...
import 'package:zappq_pharmacy/Pages/slot_freezing.dart';
...
class _SlotManagementState extends State<SlotManagement> {
  ...
  void showFreezeSlotPicker(BuildContext context) async {
    ...
    if (selectedDate != null) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => FreezeSlotGrid(dateKey: formattedDate),
        ),
      );
    }
  }
  ...
  List<String> _generateTimeSlots(bool thirtyMinutes) {
    ...
    while (startTime.isBefore(endTime)) {
      DateTime slotEnd = startTime.add(
        Duration(minutes: thirtyMinutes ? 30 : 60),
      );
  ...
  Future<void> _saveSelectedSlots() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance
          .collection('settings')
          .doc('lab_timeslots')
          .set({'times': selectedSlots}, SetOptions(merge: true));
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Timeslots updated successfully! (${selectedSlots.length} slots)',
            ),
  ...
  void _onSlotDurationChanged(bool? value) {
    ...
    setState(() {
      ...
      selectedSlots =
          selectedSlots.where((slot) => allSlots.contains(slot)).toList();
    });
  }
        ...
        actions: [
          TextButton(
            style: TextButton.styleFrom(
              ...
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), // Adjust size
            ),
            onPressed: () => showFreezeSlotPicker(context),
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: const [
                Icon(Icons.free_cancellation, color: Colors.white),
                SizedBox(width: 8),
                Text("Slot Freezing", style: TextStyle(color: Colors.white)),
              ],
            ),
          ),
...
SizedBox(width: 30,),
          ...
          if (isSaving)
            ...
            TextButton(
              style: TextButton.styleFrom(
                ...
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(30), // Square corners
                ),
              ...
              child: Row(
                ...
                children: const [
                  ...
                  SizedBox(width: 8),
                  Text("Save Timeslots", style: TextStyle(color: Colors.white)),
                ],
              ),
            ),
          ...
          // IconButton(
            ...
            //   onPressed:
            //       _saveSelectedSlots, // Always allow saving, even empty lists
